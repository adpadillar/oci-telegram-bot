...
export async function generateTaskListPDF(tasks: TaskResponse[]) {
  ...
  tasks.forEach((task, index) => {
    ...
    const description = task.description.length > 45 
    ...
    // Assigned To column
    pdf.text(task.assignedToId ? task.assignedToId.toString() : "Unassigned", xPos, yPos);
  ...
  pdf.text("Task Management System", pageWidth / 2, footerY, { align: "center" });
