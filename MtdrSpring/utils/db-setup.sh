...
CREATE TABLE $U.users (
    ...
    title        VARCHAR2(255),
    CONSTRAINT fk_users_project FOREIGN KEY (project_id) REFERENCES $U.projects(id)
);
CREATE TABLE $U.tasks (
    ...
    status         VARCHAR2(128) DEFAULT 'created' NOT NULL,
    ...
    CONSTRAINT fk_tasks_project FOREIGN KEY (project_id) REFERENCES $U.projects(id),
    CONSTRAINT fk_tasks_created_by FOREIGN KEY (created_by) REFERENCES $U.users(id),
    CONSTRAINT fk_tasks_assigned_to FOREIGN KEY (assigned_to) REFERENCES $U.users(id)
);
CREATE TABLE $U.messages (
    id            NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    ...
    user_id       NUMBER,
    created_at    TIMESTAMP(6) DEFAULT SYSTIMESTAMP NOT NULL,
    CONSTRAINT fk_messages_user FOREIGN KEY (user_id) REFERENCES $U.users(id)
);
CREATE TABLE $U.sprints (
    ...
    CONSTRAINT fk_sprints_project FOREIGN KEY (project_id) REFERENCES $U.projects(id)
);
commit;
!
